...
class Quick_prefix_dir():
    ...
    def modify_dir_name(self, dir_source):
        ...
        for root, dirs, files in os.walk(dir_source):
            ...
            for dir in dirs:
                ...
                index += 1
                self.log(f"{self.index}: {dir} -> {index_dir + dir}, in {root}")
                self.index += 1
    ...
    def run(self):
        ...
        digits = input("Input the digits of index, default: 3 (e.g. index=3, dir->002_dir)\n")
        if digits.isdigit():
            self.digits = int(digits)
        elif digits == '':
            pass
        else:
            self.log("Error Type, digits is set to 3.")
